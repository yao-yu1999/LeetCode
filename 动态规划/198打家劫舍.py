...
class Solution:
    def rob(self, nums: List[int]) -> int:
        # nums是一个存储着len(nums)间房，分别对应的金额的数组
        # 创建数组并初始化：当前选择下的最大值，当遍历到该数组的最后一个元素后，得到的就是整个最大值
        f = [0] * (len(nums)+2)  # 默认f[0]=f[1]=0

        for i, nums[i] in enumerate(nums): # 用enumerate(nums)同时获取索引i和对应的金额nums[i]
